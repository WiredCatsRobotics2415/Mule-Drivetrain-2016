 ...
 *
 */
public class AdvanceAutoScriptCommand extends CommandGroup {
	
	private final double F = 12;
	double x, y, thetaT, thetaF, distance;
	
	public AdvanceAutoScriptCommand(double x, double y){
//		this(x, y, 0);
		this.x = x*F;
    	this.y = y*F;
...
//    	addSequential(new GyroTurnCommand(thetaF));
    ...
    public AdvanceAutoScriptCommand(double x, double y, double theta) {
    	this.x = x*F;
    	this.y = y*F;
    	distance = Math.sqrt(this.x*this.x + this.y*this.y);
    	thetaT = -Math.toDegrees(Math.atan2(this.x, this.y));
